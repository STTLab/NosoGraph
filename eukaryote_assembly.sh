...
usage() {
    ...
    echo "  -asm <assembler>                    Assembler: canu or flye (case sensitive)"
    echo "  -tech <technology>                  Technology: pacbio or nanopore"
    echo "  -g|--genome-size <genome size>      Genome size (e.g., 5m, 2.6g) Not required for Flye"
    echo "  -o|--outdir <output directory>      Output directory (will be created automatically)"
...
# Check input files before doing anything
if [[ ! -f $LONG_READS ]]; then
    echo "ERROR - ${LONG_READS} file not found."
    exit 1
fi
if [[ $PILON_ITER -gt 0 ]]; then
    if [[ ! -f $READ1 ]]; then
        echo "ERROR - ${READ1} file not found."
        exit 1
    fi
    if [[ ! -f $READ2 ]]; then
        echo "ERROR - ${READ2} file not found."
        exit 1
    fi
fi
...
case $ASSEMBLER in
    "canu")
        ...
        micromamba run -n assemblers canu \
            -p assembly \
    ...
    "wtdbg2")
        ...
        micromamba run -n assemblers wtdbg2 \
            -x ${WTDGB2_PRESET} \
            -i ${LONG_READS} \
        ...
        micromamba run -n assemblers wtpoa-cns -i ${OUTDIR}/01_assembly/assembly.ctg.lay.gz -o ${contigs_fasta} -t ${THREADS} -f
        ;;
    "spades")
        if [[ -f $READ1 && -f $READ2 ]]; then
            if [[ -f $LONG_READS ]]; then
                echo "Performing hybrid assembly with SPADES"
                micromamba run -n assemblers spades.py -o ${OUTDIR}/01_assembly -1 ${READ1} -2 ${READ2} --${TECH} ${LONG_READS}
                exit_code=$?
                if [[ "$exit_code" -ne 0 ]]; then
    ...
    "flye")
        case $TECH in
            "nanopore")
                ...
                ;;
            "nanopore-hq")
                FLYE_PARAMS="--nano-hq ${LONG_READS}"
                ;;
            "pacbio")
        ...
        micromamba run -n assemblers flye --out-dir ${OUTDIR}/01_assembly --threads ${THREADS} ${FLYE_PARAMS}
        exit_code=$?
        if [[ "$exit_code" -ne 0 ]]; then
...
for ((i=0; i<=RACON_ITER; i++)); do
    ...
    if [[ "$i" -gt 0 ]]; then
        ...
        if [[ ! -s $polished_sequence ]]; then
            micromamba run -n assemblers racon -t ${THREADS} \
                -e 0.1 -q 15 \
                ${LONG_READS} \
                ${overlap_file} \
            ...
            echo "Found existing polishing ${polished_sequence}"
        fi
    fi
done
...
if [[ $PILON_ITER -gt 0 || -f $READ1 || -f $READ2 ]]; then
    ...
    for ((i=0; i<=PILON_ITER; i++)); do
        if [[ $i -gt 0 ]]; then
            ...
            micromamba run -n assemblers bwa-mem2 index -p ${OUTDIR}/02_polish/02_pilon/iter_${i}/bwa_idx ${contigs_fasta}
            micromamba run -n assemblers bwa-mem2 mem -t $HALF_THREADS ${OUTDIR}/02_polish/02_pilon/iter_${i}/bwa_idx $READ1 $READ2 | samtools sort -@ $((HALF_THREADS - 1)) > ${alignment_file}
            ...
            micromamba run -n assemblers pilon \
                -Xmx24G \
    ...
    cp ${polished_sequence} ${OUTDIR}/final_polished_assembly.fa
    micromamba run -n qc_tools checkm2 predict --threads 16 --input "${OUTDIR}/final_polished_assembly.fa" --output-directory ${OUTDIR}/checkm2 --database_path /mnt/sdb/REPOSITORY/CHECKM2DB/CheckM2_database/uniref100.KO.1.dmnd --remove_intermediates
    echo ""
    echo "DONE - Final assembly is at ${OUTDIR}/final_polished_assembly.fa"
    echo ""
fi
